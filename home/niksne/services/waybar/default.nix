...
<span color='${theme.palette.green}'>{bandwidthUpBits}</span> <span color='${theme.palette.maroon}'>{bandwidthDownBits}</span> <span color='${theme.palette.mauve}'>󰹹{bandwidthTotalBits}</span>'';
      ...
      }
      {
        ...
        user = {
          format = "<span color='${theme.palette.teal}'>{user}</span> (up <span color='${theme.palette.pink}'>{work_d} d</span> <span color='${theme.palette.blue}'>{work_H} h</span> <span color='${theme.palette.red}'>{work_M} min</span> <span color='${theme.palette.green}'>↑</span>)";
          icon = true;
        };
        "hyprland/window" = {
          format = "{title} ({class})";
